...
class Risk_register extends BE_Controller {
	...
	function template() {
		...
		$arr = ['risk' => 'risk','is_active' => 'is_active'];
		$config[] = [
			'title' => 'template_import_risk_register',
			'header' => $arr,
		];
	...
	function export() {
		ini_set('memory_limit', '-1');
		$arr = ['risk' => 'Risk','is_active' => 'Aktif'];
		$data = get_data('tbl_risk_register')->result_array();
		$config = [
			'title' => 'data_risk_register',
			'data' => $data,
			'header' => $arr,
		];
		$this->load->library('simpleexcel',$config);
